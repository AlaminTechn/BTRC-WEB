#!/usr/bin/env python3
"""
BTRC QoS Monitoring - Data Loader
...
def load_table(conn, config, data, prefix):
    ...
    # For tables with ID, use ON CONFLICT
    if 'id' in columns:
        sql = f"""
...
def main():
    print("=" * 60)
    print("  BTRC QoS POC - Data Loader")
    print("=" * 60)
    ...
    try:
        ...
        print("  docker-compose up -d btrc-db")
        return False

    # Load files in correct order (P → A → B → C → D → E)
    # Custom sort: P comes first, then alphabetical
    def sort_key(x):
        if x.startswith('P.'):
            return '0' + x  # P files come first
        return '1' + x  # Others follow alphabetically
